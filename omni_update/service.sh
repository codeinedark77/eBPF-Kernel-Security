...
mount -t bpf bpf /data/local/ubuntu/sys/fs/bpf
...
# See the same fix in magisk_module/service.sh (this file is a duplicate of it) --
# 0.0.0.0 had no auth and accepted WebSocket connections from any origin. Restored
# to the loopback-only default main.go itself recommends.
